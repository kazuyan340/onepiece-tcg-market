...
(function () {
  ...
  const THRESHOLDS_KEY = "onepieceTcgPriceThresholds";
    ...
    for (const card of checkedCards) {
      ...
      link.innerHTML = `
        ...
        <div class="price-check-info">
          ...
          <div class="sub">${escapeHtml(card.rarity || "")} / ${escapeHtml(card.color || "")} / ${card.id}</div>
        </div>
      `;
      ...
      tableWrap.innerHTML = priceInfo
        ? `${shopTableHtml(card, priceInfo)}${purchaseButtonsHtml(card.name, card.id, card.rarity)}`
        : purchaseButtonsHtml(card.name, card.id, card.rarity);
